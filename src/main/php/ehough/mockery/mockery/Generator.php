...
class ehough_mockery_mockery_Generator
{
        ...
        $partialMethods = array())
    {
        ...
        foreach ($classData as $i=>$data) {
            if ($data['class']->isInterface() && preg_match("/^Traversable$/i", $data['class']->getName())) {
                ...
                $inheritance = ' extends ' . $data['className'] . ' implements ehough_mockery_mockery_MockInterface';
                $classNameInherited = $data['className'];
            }
        }
        if (count($interfaceInheritance) > 0) {
            ...
            if (!$classIsFinal) $interfaceInheritance[] = 'ehough_mockery_mockery_MockInterface';
    ...
    protected static function _analyseClass($class, $className, $allowFinal = false)
    {
        if ($class->isFinal() && !$allowFinal) {
            throw new ehough_mockery_mockery_Exception(
                ...
                . ' to ehough_mockery_Mockery::mock() as instantiated objects to create a'
                . ' partial mock, but only if the mock is not subject to type'
                . ' hinting checks.'
            );
        } elseif ($class->isFinal()) {
            $className = 'ehough_mockery_mockery_Mock';
        }
...
BODY;
    ...
    protected static function _renderPublicMethodParameters(ReflectionMethod $method)
    {
        ...
        if ($class->isInternal()) { // check for parameter overrides for internal PHP classes
            $paramMap = ehough_mockery_Mockery::getConfiguration()
    ...
    {
        try {
            \$associatedRealObject = ehough_mockery_Mockery::fetchMock(__CLASS__);
...
MOCK;
    ...
    public function __construct()
    {
        ...
        foreach (\$directors as \$method=>\$director) {
            ...
            if (!\$existingDirector) {
                \$existingDirector = new ehough_mockery_mockery_ExpectationDirector(\$method, \$this);
        ...
        ehough_mockery_Mockery::getContainer()->rememberMock(\$this);
    }
MOCK;
            $std .= $mim;
        }
