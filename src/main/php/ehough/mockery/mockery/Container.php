...
class ehough_mockery_mockery_Container
{
    const BLOCKS = ehough_mockery_Mockery::BLOCKS;
    ...
    public function mock()
    {
        ...
        while (count($args) > 0) {
            ...
            if (is_string($arg) && strpos($arg, ',') && !strpos($arg, ']')) {
                ...
                if (!class_exists($parts[0], true) && !interface_exists($parts[0], true)) {
                    throw new ehough_mockery_mockery_Exception('Can only create a partial mock from'
        ...
        if (!is_null($name) && !is_null($class)) {
            if (!$makeInstanceMock) {
                $mockName = ehough_mockery_mockery_Generator::createClassMock($class, null, null, $blocks);
            } else {
                $mockName = ehough_mockery_mockery_Generator::createClassMock($class, null, null, $blocks, true);
            }
            $result = class_alias($mockName, $name);
            $mock = $this->_getInstance($name, $constructorArgs);
            $mock->mockery_init($class, $this);
        } elseif (!is_null($name)) {
            $mock = new ehough_mockery_mockery_Mock();
            $mock->mockery_init($name, $this);
        } elseif(!is_null($class)) {
            $mockName = ehough_mockery_mockery_Generator::createClassMock($class, null, null, $blocks, false, $partialMethods);
            $mock = $this->_getInstance($mockName, $constructorArgs);
            $mock->mockery_init($class, $this);
        } elseif(!is_null($partial)) {
            $mockName = ehough_mockery_mockery_Generator::createClassMock(get_class($partial), null, true, $blocks);
            $mock = $this->_getInstance($mockName, $constructorArgs);
     ...
     * @return void
     */
    public function mockery_validateOrder($method, $order, ehough_mockery_mockery_MockInterface $mock)
    {
        if ($order < $this->_currentOrder) {
            $exception = new ehough_mockery_mockery_exception_InvalidOrderException(
     ...
     *
     * @param ehough_mockery_mockery_Mock
     * @return ehough_mockery_mockery_Mock
     */
    public function rememberMock(ehough_mockery_mockery_MockInterface $mock)
    {
